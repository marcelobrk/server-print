...
namespace Dicom.Printing
{
   ...
   public class PrintJob : DicomDataset
   {
      ...
      private IRestResponse GetPrinter(object AET){

         var client = new RestClient("http://localhost:6363/printer/" + AET + "");
      ...
      private void DoPrint()
      {
         ...
         var response = GetPrinter($"{PrintService.CallingAE}");
         ...
         if (json.printer == 0)
         {
            Console.WriteLine("Conexao recusada");
         ...
         else
         {
               ...
               try
               {
                  ...
                  dynamic printerSettings = new PrinterSettings
                  {
                     PrinterName = json.printer,
                     PrintToFile = true,
                     PrintFileName = Path.Combine(FullPrintJobFolder, SOPInstanceUID.UID + ".pdf")
                  };
  


                  Console.WriteLine(printerSettings);
      ...
      {
         _currentFilmBox.Print(e.Graphics, e.MarginBounds, 300);
      ...
      private void OnQueryPageSettings(object sender, QueryPageSettingsEventArgs e)
      {
         ...
         e.PageSettings.Margins.Left = 2;
         e.PageSettings.Margins.Right = 2;
         e.PageSettings.Margins.Top = 2;
         e.PageSettings.Margins.Bottom = 2;
